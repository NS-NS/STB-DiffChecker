...
{
    public partial class StbBeam : ICompare, IProperty
    {
        ...
        public bool AlmostCompareTo(object obj, IST_BRIDGE istbA, IST_BRIDGE istbB, IToleranceSetting itoleranceSetting)
        {
            ...
            return Math.Abs(startA.X - startB.X) < toleranceSetting.BeamTolerance.Node &&
                   Math.Abs(startA.Y - startB.Y) < toleranceSetting.BeamTolerance.Node &&
                   Math.Abs(startA.Z - startB.Z) < toleranceSetting.BeamTolerance.Node &&
                   Math.Abs(endA.X - endB.X) < toleranceSetting.BeamTolerance.Node &&
                   Math.Abs(endA.Y - endB.Y) < toleranceSetting.BeamTolerance.Node &&
        ...
        public void CompareProperty(PropertyInfo info, IST_BRIDGE istbA, object objB, IST_BRIDGE istbB, string parentElement, List<string> key, List<Record> records, Dictionary<string, Importance> importanceDict, IToleranceSetting itoleranceSetting)
        {
                      ...
                      "offset_end_Z")
            {
                ...
                Consistency consistency = Math.Abs(offsetA - offsetB) < Utility.Tolerance
                    ...
                    : Math.Abs(offsetA - offsetB) < toleranceSetting.BeamTolerance.Offset ? Consistency.AlmostMatch : Consistency.Inconsistent;
                records.Add(new Record(
                    parentElement,
                    key,
                    "@" + info.Name,
        ...
        private string FindBeamName(string id, string kind, ST_BRIDGE stbridge)
        {
            if (kind == "RC")
            {
                ...
                return beam == null ? null : beam.floor + "/" + beam.name;
            }
            else if (kind == "S")
            {
