...
using STB_DiffChecker_210;
using System.Reflection;

namespace ST_BRIDGE210
{
    public partial class StbPost : ICompare, IProperty
    {
        ...
        public bool AlmostCompareTo(object obj, IST_BRIDGE istbA, IST_BRIDGE istbB, IToleranceSetting itoleranceSetting)
        {
            ...
            return Math.Abs(bottomA.X - bottomB.X) < toleranceSetting.PostTolerance.Node &&
                   Math.Abs(bottomA.Y - bottomB.Y) < toleranceSetting.PostTolerance.Node &&
                   Math.Abs(bottomA.Z - bottomB.Z) < toleranceSetting.PostTolerance.Node &&
                   Math.Abs(topA.X - topB.X) < toleranceSetting.PostTolerance.Node &&
                   Math.Abs(topA.Y - topB.Y) < toleranceSetting.PostTolerance.Node &&
                   Math.Abs(topA.Z - topB.Z) < toleranceSetting.PostTolerance.Node;
        }
        ...
        public void CompareProperty(PropertyInfo info, IST_BRIDGE istbA, object objB, IST_BRIDGE istbB, string parentElement, List<string> key, List<Record> records, Dictionary<string, Importance> importanceDict, IToleranceSetting itoleranceSetting)
        {
                 ...
                 "offset_top_Z")
            {
                ...
                Consistency consistency = Math.Abs(offsetA - offsetB) < Utility.Tolerance
                    ...
                    : Math.Abs(offsetA - offsetB) < toleranceSetting.PostTolerance.Offset ? Consistency.AlmostMatch : Consistency.Inconsistent;
                records.Add(new Record(
                    parentElement,
                    key,
                    "@" + info.Name,
