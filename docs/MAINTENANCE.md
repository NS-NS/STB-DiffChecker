...
**設計判断の経緯と理由は[docs/adr/](adr/README.md)を参照**(なぜリフレクションなのか、
なぜ統合exeなのか等は全部そちらに書いてある)。
...
```powershell
.\tools\run-regression.ps1
```

- tools/DiffCheckerHarness(ヘッドレス比較ツール)で標準ケースを実行し、
  全レコードのダンプをtools/baselines/の正解データとハッシュ照合する
- ケース: 2.0.2実データ(TestData/FileA・B)、2.1ミニデータ(許容差100mm、
  「5mmずらした大梁が許容差内になる」ことを含む — 過去にこの判定が数年間無効だった)
- **意図した挙動変更**でFAILした場合は、`fc`で差分内容を目視確認してから
  `.\tools\run-regression.ps1 -UpdateBaseline` でベースラインを更新し、差分の説明を
  コミットメッセージに書く
- 注: ダンプは数値のToString結果を含むため、カルチャ設定が特殊な環境では
  差分が出る可能性がある(その場合は内容を確認)

**ハーネスの単体利用(任意のファイルで動作確認):**

```powershell
dotnet run --project tools\DiffCheckerHarness -c Release -- 210 A.stb B.stb 100
# 引数: <バージョン> <fileA> <fileB> [許容差mm] [ダンプ出力パス]
```

GUIを起動せずに読込〜比較〜タブ別集計まで確認できる。新バージョン対応時の
スモークテストや、性能計測(compare timeを出力)にも使う。

**その他:**
- バージョン層に手を入れたら、該当バージョンの実ファイルでハーネスを通す
- UI(WPF/Blazor)に手を入れたら、実アプリで一連の操作(読込→実行→フィルタ→出力)を通す
