...
1. "STB-DiffChecker"を起動し、「入力・設定」タブ上で比較したいstbファイルをそれぞれ選択する
    * ST-Bridgeのバージョン(2.0.1 / 2.0.2 / 2.1.0)はファイルから自動判別され、タイトルバーに表示される
    * ファイルAとファイルBは同一バージョンである必要がある
2. 「重要度」タブで設計段階プロファイル(S2 / S4 / カスタム)を選択する
    * stbファイルの読み込みでバージョンが確定すると、既定でS2プリセットが適用される
    * S2 / S4は各バージョンに同梱のプリセット。カスタムは設定ファイル(csv)から読み込む
    * セルはドロップダウン(高/中/低/対象外)で個別に編集でき、プロファイル適用時から変更したセルは着色され、選択表示が「S2\*」のようになる
    * 別バージョンのstbファイルを読み込み直すと、S2/S4は同プロファイルを再適用(編集は破棄)、カスタムはS2に戻る
3. (必要に応じて)許容差を修正する
4. 「実行」を押す 
5. 結果出力用のタブが作成されるので選択する
6. フィルタを設定して必要な情報を確認する
7. (必要に応じて）Excelに出力して確認する
* 編集した重要度は「設定を書き出す」でcsvに保存し、次回「カスタム」または「設定ファイルを読込む」で再利用できる

## 使用方法（Web版）
1. [GitHub Pages](https://ns-ns.github.io/STB-DiffChecker/)を開く（初回のみ実行環境のダウンロードで数十秒かかる）
2. ファイルA・ファイルBを選択する（バージョンは自動判別。ファイルはブラウザ内で処理され、サーバーには送信されない）
3. (必要に応じて)許容差の修正、重要度プリセット(S2/S4)の適用、設定ファイル(csv)の読み込みを行う
    * 重要度の個別編集はデスクトップ版のみ対応
4. 「実行」を押すと概要と結果詳細が表示される（検索はEnterで確定）
5. (必要に応じて)結果をCSV出力する
